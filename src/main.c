...
void *gameThread(void *vargp) {
    ...
    while(1){
        data[0]=1;
        printf("Printing from Thread 1 %d %d %d\n", data[0], data[1], data[2]);
        sleep(2);
    }
    return NULL;
}

void *knobsThread(void *vargp) {
    sleep(1);
    while(1){
        data[1]=2;
        printf("Printing from Thread 2 %d %d %d\n", data[0], data[1], data[2]);
        sleep(2);
    }
    return NULL;
}
...
void *serverThread(void *vargp) {
    ...
    while(1){
        n++;
        data[2] = n;
        printf("Printing from Thread 3 %d %d %d\n", data[0], data[1], data[2]);
        sleep(2);
    }
    return NULL;
}
...
int main(int argc, char *argv[]) {
    ...
    // fillCell(8, 11, 0xF054);
    redraw(mem);
    unsigned char *knob_mem=init_knobs();
