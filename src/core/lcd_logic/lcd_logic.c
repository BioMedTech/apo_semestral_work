        ...
        for (int j = 0; j < 8; j++)
        {
            ...
            for (int m=0; m < scale; m++){
                for (int n=0; n < scale; n++){
                  data[((i + row * 16) * scale + m) * WIDTH + (column * 8 + j)*scale+n] = (font_rom8x16.bits[(int)letter * 16 + i] >> (15 - j)) & 1 ? color : bg;
                }
            }
        }
    }
...
void drawString(char *str, int row, int column, int16_t color, uint16_t bg, int scale)
{
    for (int i=0; i < strlen(str); i++){
        drawLetter(str[i], row, column+i, color, bg, scale);
    }
}
...
void fillBgImg(char *filename)
{
    ...
    for (i = 0; i < rows; i++){
        for (j = 0; j < cols; j++){
            uint16_t color = ((img[(cols * i + j) * 3] & 0xf8) << 8 ) | ((img[(cols * i + j) * 3 + 1] & 0xfc) << 3) | (img[(cols * i + j) * 3 + 2] >> 3);
